...
async function readAppearanceState(): Promise<ToggleTileState> {
  ...
  const raw = (
    await execAsync(["gsettings", "get", "org.gnome.desktop.interface", "color-scheme"])
  ).trim();
...
async function readUpdatesState(): Promise<InfoTileState> {
  ...
  return {
    line1: "Update check error",
    line2: text,
    detail: `Unexpected update response: ${text}`,
    error: `Unexpected update response: ${text}`,
  } satisfies InfoTileState;
}
...
function computeBatteryEta(
  ...
  if (state === "charging") return timeToFull ? `to full ${timeToFull}` : "charging";
  if (state === "discharging") return timeToEmpty ? `remaining ${timeToEmpty}` : "discharging";
...
function setupStandardPolls(markUpdated: () => boolean, markFirstPollComplete: () => boolean) {
...
function createControlCenterService(): ControlCenterService {
  ...
  const { claudeUsageDataPoll, volumePoll, brightnessPoll, batteryPoll, refreshUsage } = auxPolls;
  ...
  const runSystemAction = async (label: string, action: () => Promise<boolean>) => {
    try {
      await action();
      clearActionError();
    } catch (error) {
