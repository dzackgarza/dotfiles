...
const USAGE_LIMITS_DIR = resolve(import.meta.dirname, "../../../../../usage-limits");

let data: UsageCollection;

beforeAll(() => {
  const output = execSync(`uv run --project "${USAGE_LIMITS_DIR}" usage-limits --json`, {
    encoding: "utf-8",
    timeout: 30_000,
  });
...
describe("usage-limits --json", () => {
  ...
  it("every provider from CLI output has an icon in PROVIDER_ICONS", () => {
    ...
    const missing = data.providers.filter((p) => !iconKeys.has(p.provider)).map((p) => p.provider);
