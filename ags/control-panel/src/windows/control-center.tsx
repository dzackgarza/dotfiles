...
import { getControlCenterService } from "../services/control-center";
  ...
  return (
    ...
    >
      <For each={claudeUsageData((d) => d.providers)}>
        {(provider: ProviderSnapshot) => <ProviderIcon provider={provider} onClicked={onClicked} />}
      </For>
    </box>
  );
}
...
export function ControlCenterWindow() {
  ...
  const tileRows: JSX.Element[][] = [
    ...
    [
      <InfoTile icon="xsi-software-update-available-symbolic" state={control.updates} />,
      <InfoTile
    ...
    [<UsageTile icon="xsi-drive-harddisk-symbolic" state={control.disk} />, <box />],
  ];
  ...
  const panel = (
    <box class="nc-root" orientation={Gtk.Orientation.VERTICAL} spacing={12}>
      ...
      />
      <box
        class={control.battery((value) => joinClasses("status-row", value.error && "error"))}
        orientation={Gtk.Orientation.HORIZONTAL}
        spacing={12}
