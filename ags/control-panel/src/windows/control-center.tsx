...
function getControlCenterMaxHeight(): number {
  try {
    const m = Gdk.Display.get_default()
      ?.get_monitors()
...
function getProviderTooltipText(provider: ProviderSnapshot): string {
  ...
  if (provider.rows?.length) {
    for (const row of provider.rows) {
      const resets = row.time_until_reset
        ? ` (resets ${row.time_until_reset})`
        : ""
      parts.push(`• ${row.identifier}: ${Math.round(row.pct_used)}%${resets}`)
    }
...
function getProviderSubLabel(provider: ProviderSnapshot): string | null {
  if (
    ...
    provider.metadata &&
    typeof provider.metadata.total_balance === "string"
  ) {
