  ...
  return (
    ...
    >
      ...
      <Gtk.ProgressBar
        ...
        fraction={Math.min(Math.max(Math.round(percentage) / 100, 0), 1)}
        hexpand
        halign={Gtk.Align.FILL}
      />
...
export function ClaudeUsagePopover({
  ...
  const popoverContent = (
    ...
    >
              ...
              {(provider: ProviderSnapshot) => (
                <box orientation={Gtk.Orientation.VERTICAL} spacing={0}>
                  ...
                  >
                    ...
                    />
                    {provider.account && provider.account !== "default" && (
                      <label
                  ...
                  {provider.errors?.length ? (
                    ...
                    >
                      <label
                        ...
                        xalign={0}
                        label={
                          provider.errors[0].message || "Failed to fetch usage"
                        }
                      />
                    </box>
                  ) : (
                    <box orientation={Gtk.Orientation.VERTICAL} spacing={0}>
                      {provider.rows.map((row) => {
                        ...
                        if (Math.round(row.pct_used) >= 80) {
                          color = "rgb(239, 68, 68)" // red
                        } else if (Math.round(row.pct_used) >= 50) {
      ...
      >
        <label
          class="popover-updated-text"
          xalign={0}
          label="Updated 0 sec ago"
          hexpand
        />
        <button
          class="popover-refresh-btn"
