...
from src.core.mock_scenarios import EnhancedMockCognitionPlugin, CognitionStep
from src.core.animation_clock import AnimationClock

# Skip these tests for now - widget mounting issues need app context redesign
pytestmark = pytest.mark.skip(reason="Widget tests need app context redesign")
...
class TestCognitionWidgets:
    ...
    @pytest.mark.asyncio
    async def test_cognition_step_widget_creation(self):
        ...
        # Create widget in app context
        class TestApp(App):
            def compose(self):
                yield CognitionStepWidget(sub_module)

        app = TestApp()
        async with app.run_test() as pilot:
            widget = app.query_one(CognitionStepWidget)
            assert widget.sub_module == sub_module

    @pytest.mark.asyncio
    async def test_cognition_step_widget_state_updates(self):
    ...
    def test_cognition_pipeline_widget_creation(self):
        ...
        plugin.get_current_status = MagicMock(
            return_value={
                "state": "pending",
                "progress": 0.0,
                "total_tokens": 0,
...
class TestUXPolishIntegration:
    ...
    def test_widget_reactive_properties(self):
        ...
        step = CognitionStep(name="Test", icon="🧪", model="test", description="Test")
        sub_module = MagicMock()
        sub_module.step = step
        sub_module.state = "pending"
        sub_module.progress = 0.0
