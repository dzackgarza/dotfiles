...
def track_edit_usage(tool_name):
    """Track Edit tool usage and notify at 10, 20, 30, etc."""
    if tool_name not in ['Edit', 'Write', 'MultiEdit', 'Update']:
        return
        
    # Get project root and counter file path using absolute path
    hook_dir = Path(__file__).parent.absolute()
    project_root = hook_dir.parent.parent
    counter_file = project_root / 'logs' / 'edit_count.txt'
    ...
    # Notify every 10th edit
    if count % 10 == 0:
        import subprocess
        subprocess.run([
            'notify-send', 
            f'Edit Tool Usage', 
            f'Claude has made {count} code edits this session'
        ])
...
def main():
    try:
        ...
        # Debug: Log all tool names to help identify correct one
        debug_file = Path('/tmp/post_hook_tools.txt')
        with open(debug_file, 'a') as f:
            f.write(f"{tool_name}\n")
        
        # Track Edit tool usage  
        track_edit_usage(tool_name)
