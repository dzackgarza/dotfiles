...
from ui.dedicated_input_box import DedicatedInputBox, SimpleInputBox
from ui.bulletproof_input_box import create_bulletproof_input_box
...
class StructurallyCorrectREPL:
    ...
    def __init__(self, config_name: str = "debug"):
        ...
        # Initialize bulletproof input box (prevents graphical bugs)
        self.input_box = create_bulletproof_input_box(raw_console)
    ...
    async def run_startup_sequence_with_proof(self) -> StartupComplete:
        ...
        # Run startup sequence with proof of completion (plugins already displayed)
        startup_proof = await self.startup_manager.create_startup_proof(len(startup_plugins))
    ...
    async def process_user_input(self, user_input: str) -> None:
        ...
        try:
            ...
            # Add to secure timeline
            context = RenderContext(display_mode="inscribed")
            render_data = await user_input_plugin.render(context)
            timeline_plugin = create_timeline_eligible_plugin(user_input_plugin, processing_start)
            self.app.add_plugin_to_timeline(timeline_plugin)
            ...
            timeline_plugin = create_timeline_eligible_plugin(cognition_plugin, processing_start)
            self.app.add_plugin_to_timeline(timeline_plugin)
            ...
            timeline_plugin = create_timeline_eligible_plugin(assistant_plugin, processing_start)
            self.app.add_plugin_to_timeline(timeline_plugin)
    ...
    async def run_interactive_mode(self) -> None:
        ...
        # NO PROMPT: In pure timeline architecture, prompts would also be plugins
        # User input appears directly as User_Input blocks without raw prompts
        
        try:
            while self.running:
                # Get user input using dedicated input box
                try:
                    user_input = await self.input_box.get_simple_input()
                    ...
                    if user_input.lower() in ['exit', 'quit', 'bye', '/quit', '/exit']:
                        # Create a goodbye plugin for clean timeline exit
                        goodbye_id = await self.plugin_manager.create_plugin("system_check", {
                            "timeout_seconds": 1,
                            "required_checks": ["Session Cleanup"],
                            "fail_fast": False
                        })
                        
                        goodbye_plugin = self.plugin_manager.active_plugins[goodbye_id]
                        processing_start = time.time()
                        
                        # Process goodbye with session cleanup
                        await goodbye_plugin.process([
                            {"name": "Session Cleanup", "type": "cleanup_check"}
                        ], {})
                        
                        # Create a specialized goodbye adapter
                        from plugin_timeline_adapter import PluginTimelineAdapter
                        
                        class GoodbyeAdapter(PluginTimelineAdapter):
                            def get_rendered_content(self):
                                ...
                                return Panel(
                                    ...
                                    title=f"🎯 Goodbye ✅ ({metadata.wall_time_seconds:.1f}s)",
                                    box=ROUNDED,
                                    border_style="green"
                                )
                        
                        goodbye_adapter = GoodbyeAdapter(goodbye_plugin, processing_start)
                        
                        # Validate and add to timeline
                        if goodbye_adapter.validate_timeline_eligibility():
                            self.app.add_plugin_to_timeline(goodbye_adapter)
                    ...
                    # Continue to next input (input box handles its own state)
