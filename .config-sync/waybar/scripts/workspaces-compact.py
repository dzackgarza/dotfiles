...
def _workspace_item(workspace_id: int, windows: int, active_id: int) -> str:
    label = str(workspace_id) if windows == 0 else f"{workspace_id}:{windows}"
    if workspace_id == active_id:
        return (
            f"<span foreground='{ACTIVE_FG}' background='{ACTIVE_BG}'>"
            f"{html.escape(label)}</span>"
        )
    if windows == 0:
        return f"<span foreground='{DIM}'>{workspace_id}</span>"
    return (
        f"<span foreground='{FG}'>{workspace_id}</span>"
        f"<span foreground='{DIM}'>:</span>"
        f"<span foreground='{COUNT}'>{windows}</span>"
    )
...
def render_payload(workspaces: list[dict], active_workspace: dict) -> dict:
    ...
    return {
        "text": "<span foreground='#3e4b59'> </span>".join(items),
        "tooltip": "\n".join(tooltip_lines),
