...
"""
        ...
        if isinstance(result['result'], dict) and 'image_file' in result['result']:
            ...
            # Otherwise, verify the result is 9 (2^2 + 2*2 + 1 = 9)
            self.assertEqual(result['result'], 9, 
                          f"Expected 9, got {result['result']}")
    ...
    def test_plotting(self):
        ...
        """
        result = execute_sage_code(code)
        self.assertTrue(result['success'], f"Plot generation failed: {result.get('error', '')}")
        ...
        if 'PLOT_RESULT:' in output:
            ...
            self.assertIn('PLOT_RESULT:', output, "Could not find plot result in output")
        else:
            # Fallback to checking the result directly
            self.assertIsInstance(result.get('result'), dict, "Expected a dictionary result")
            plot_data = result['result']
            self.assertEqual(plot_data.get('type'), 'plot')
            self.assertEqual(plot_data.get('title'), 'Parabola')
            self.assertTrue(plot_data.get('exists', False), 
    ...
    def test_calculus_operations(self):
        ...
        self.assertTrue(result['success'], f"Differentiation failed: {result.get('error', '')}")
        ...
        self.assertTrue(result['success'], f"Integration failed: {result.get('error', '')}")
        ...
        if 'INTEGRAL_RESULT:' in output:
            ...
            self.assertEqual(result_value, '7/3', 
